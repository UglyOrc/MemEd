...
SURFACE     = "#16161f"      # panels / sidebar
SURFACE2    = "#1e1e2a"      # cards, treeview rows
BORDER      = "#2a2a3a"      # dividers
INPUT_BG    = "#1a1a26"      # entry / combobox fill

BLUE        = "#5b9cf6"      # primary accent
BLUE_DIM    = "#3a5fa0"      # hover / secondary
GREEN       = "#4ec994"      # success / freeze
ORANGE      = "#e8934a"      # frozen indicator
RED         = "#e05c6a"      # danger / warn
YELLOW      = "#d4c06a"      # changed value

TEXT        = "#d4d4dc"      # primary text
TEXT_DIM    = "#5a5a72"      # secondary / hints
TEXT_BRIGHT = "#ffffff"      # headings
...
def _btn(parent, text, cmd, kind="default", **kw):
    """Unified button factory with three visual tiers."""
    styles = {
...
class App(tk.Tk):
    def __init__(self):
        ...
        self.after(200, self._apply_window_stealth)
    ...
    def _build_menu(self):
        ...
        stm = _menu()
        stm.add_command(label="Stealth Settings...",           command=self._open_stealth_panel)
        stm.add_command(label="Re-apply Debug Flag Clear",     command=self._reapply_stealth)
        mb.add_cascade(label="Stealth", menu=stm)
    ...
    def _build_sidebar(self, parent):
        ...
        # Value inputs (shown/hidden by mode)
    ...
    def _build_right(self, parent):
        # Split into scan results (top) and address table (bottom)
    ...
    def _build_addr_table(self, parent):
        ...
        # File buttons (right side of header)
        _btn(hdr, "💾", self._save_address_list,
             kind="ghost", font=UI_SMALL, padx=8, pady=2
             ).pack(side=tk.RIGHT, padx=1)
        _btn(hdr, "📂", self._load_address_list,
             kind="ghost", font=UI_SMALL, padx=8, pady=2
             ).pack(side=tk.RIGHT, padx=1)
    ...
    def _open_process(self):
        ...
        if self._engine.attach(dlg.selected_pid):
            ...
            messagebox.showerror(
                ...
                "Try running MemEd as Administrator.", parent=self)
    ...
    def _edit_addr_value(self, _=None):
        ...
        if self._engine.write_value(row.address, nv, row.vtype):
            ...
            messagebox.showerror("Write Failed",
                                  f"Could not write 0x{row.address:016X}.", parent=self)
        self._refresh_addr_table()
    ...
    def _show_about(self):
        messagebox.showinfo("About MemEd",
            ...
            "Stealth: Minimal access · NT direct R/W · Hide window\n\n"
            "F11 = Pause freeze    F12 = Unfreeze all\n"
            "Ctrl+S = Save    Ctrl+O = Load\n\n"
            "Run as Administrator for full access.", parent=self)
    ...
    def _suggested_filename(self) -> str:
        ...
        proc = (self._current_proc_name or "addresses").replace(".exe", "")
    ...
    def _apply_window_stealth(self):
        if self._engine.stealth.hide_window:
            try:
                hide_window(self.winfo_id())
            except Exception:
                pass
        self._update_stealth_badge()

    def _update_stealth_badge(self):
        cfg   = self._engine.stealth
        any_on = any([cfg.use_minimal_access, cfg.clear_debug_flag,
    ...
    def _reapply_stealth(self):
        ...
        messagebox.showinfo("Stealth",
            ...
            "Could not clear flags (need higher privileges).", parent=self)

    def _open_stealth_panel(self):
        StealthPanel(self, self._engine.stealth,
                     on_change=lambda: (self._update_stealth_badge(),
                                        self._apply_window_stealth()))
...
class FreezeSafetyPanel(tk.Toplevel):
    ...
    def _refresh_loop(self):
        ...
        for addr, e in entries.items():
            if e.disabled:
                status, tag = f"✕ {e.last_error[:18]}", ("disabled",)
            elif e.error_count:
...
class StealthPanel(tk.Toplevel):
    def __init__(self, parent, config, on_change=None):
        super().__init__(parent)
        self.title("Stealth Settings")
        self.configure(bg=BG)
        self.resizable(False, False)
        self.transient(parent)
        self.grab_set()
    ...
    def _build(self):
        hdr = tk.Frame(self, bg=SURFACE, pady=12, padx=16)
        hdr.pack(fill=tk.X)
        tk.Label(hdr, text="Anti-Detection Settings", font=UI_TITLE,
                 fg=TEXT_BRIGHT, bg=SURFACE).pack(side=tk.LEFT)
        ...
        OPTIONS = [
            ("use_minimal_access", "Minimal Access Rights",
             "Open with VM_READ/WRITE instead of PROCESS_ALL_ACCESS.\n"
             "Avoids common AC handle-rights signature."),
            ("clear_debug_flag",   "Clear PEB Debug Flag",
             "Clear BeingDebugged so IsDebuggerPresent() returns false.\n"
             "Applied on attach and via re-apply."),
            ("use_nt_read_write",  "NT Direct Read / Write",
             "Use NtReadVirtualMemory directly, bypassing kernel32 hooks."),
            ("hide_window",        "Hide Window from Capture APIs",
             "SetWindowDisplayAffinity(EXCLUDE_FROM_CAPTURE)."),
            ("random_scan_delay",  "Random Scan Delay  (1–5 ms)",
             "Adds jitter between region reads to defeat timing heuristics."),
        ]

        self._vars: dict[str, tk.BooleanVar] = {}
        self._status_labels: list[tk.Label]  = []

        for attr, label, desc in OPTIONS:
            f = tk.Frame(body, bg=SURFACE2, padx=12, pady=8)
            ...
            tk.Checkbutton(f, text=label, variable=var,
                           bg=SURFACE2, fg=TEXT, selectcolor=INPUT_BG,
                           activebackground=SURFACE2, activeforeground=BLUE,
                           font=UI_FONT, command=self._apply).pack(anchor=tk.W)
            tk.Label(f, text=desc, font=UI_SMALL, fg=TEXT_DIM,
                     bg=SURFACE2, justify=tk.LEFT).pack(anchor=tk.W, padx=20)

        tk.Frame(body, bg=BORDER, height=1).pack(fill=tk.X, pady=10)
        tk.Label(body, text="STATUS", font=UI_SMALL, fg=TEXT_DIM, bg=BG).pack(anchor=tk.W)
        for item in get_stealth_status(self._cfg):
            lbl = tk.Label(body, text="", font=UI_SMALL, bg=BG, anchor=tk.W)
            lbl.pack(anchor=tk.W, padx=6, pady=1)
    ...
    def _apply(self):
        for attr, var in self._vars.items():
            setattr(self._cfg, attr, var.get())
        self._refresh_status()
        if self._on_change:
            self._on_change()

    def _refresh_status(self):
        for lbl, (name, on, _) in zip(self._status_labels,
                                       get_stealth_status(self._cfg)):
            lbl.config(text=f"{'✔' if on else '✘'}  {name}",
